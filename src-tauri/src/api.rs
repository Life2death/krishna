...
// Naukri Lelo Prompts API
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct NaukriPrompt {
    title: String,
    prompt: String,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct NaukriPromptsResponse {
    prompts: Vec<NaukriPrompt>,
    total: i32,
...
// Fetch Naukri Lelo Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<NaukriPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: NaukriPromptsResponse = response
        .json()
        .await
